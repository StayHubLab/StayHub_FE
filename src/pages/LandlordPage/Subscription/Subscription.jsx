...
const Subscription = () => {
  const plans = [
    ...
    },
    {
      ...
      price: 299000,
      features: ['Không giới hạn phòng', 'Hỗ trợ 24/7', 'Báo cáo chi tiết', 'Tích hợp thanh toán'],
      popular: true,
    },
    {
      name: 'Gói doanh nghiệp',
      price: 599000,
      features: ['Tất cả tính năng Pro', 'Quản lý nhiều tòa nhà', 'API tích hợp', 'Ưu tiên hỗ trợ'],
    },
  ];
      ...
      <Row gutter={[24, 24]} className="subscription-plans">
        {plans.map((plan, index) => (
          <Col xs={24} md={8} key={index}>
            ...
            >
              {plan.popular && <Tag color="orange" className="popular-tag">Phổ biến</Tag>}
              <div className="plan-header">
                <CrownOutlined className="plan-icon" />
                <Title level={4}>{plan.name}</Title>
                <div className="plan-price">
                  <span className="price">{plan.price === 0 ? 'Miễn phí' : `${plan.price.toLocaleString()}VNĐ`}</span>
